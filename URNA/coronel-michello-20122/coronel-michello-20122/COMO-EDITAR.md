...
Abra `assets/js/dados.js` e procure o bloco `videos:`.
...
## 2. Publicar depoimentos

> ⚠️ **Leia isto antes.** Os quatro depoimentos que estão no site agora **não são falas de
> pessoas reais** — são modelos de diagramação, para você ver como fica o desenho.
> Por isso aparecem com uma tarja laranja **EXEMPLO — SUBSTITUIR**. É uma trava proposital:
> ninguém vai publicar depoimento inventado por engano.

Antes de trocar por um depoimento de verdade, garanta os três passos:

1. Grave ou colete o depoimento real.
2. **Obtenha autorização por escrito** de uso de nome e imagem da pessoa.
3. Só então publique.

Em `assets/js/dados.js`, no bloco `depoimentos:`, cada item é assim:

```js
{
  exemplo: true,
  texto: "Espaço reservado para o depoimento de um morador…",
...
1. Troque `texto`, `nome` e `papel`.
2. **Apague a linha `exemplo: true,` inteira** — é ela que gera a tarja laranja.

Ficando assim:

```js
{
  texto: "Depoimento real da pessoa, transcrito aqui.",
  nome: "Maria Souza",
  papel: "Moradora — Ceilândia"
}
```

O círculo com as iniciais é gerado automaticamente a partir do nome.

**Quando terminar de trocar todos**, apague também o bloco de aviso amarelo que aparece
embaixo do carrossel: abra `index.html`, procure por `nota nota--revisar` dentro da
seção `#depoimentos` e apague o parágrafo inteiro.

---

## 3. Acrescentar uma matéria de imprensa

Em `assets/js/dados.js`, bloco `midia:`. Copie um item, cole e preencha:
...
As **quatro primeiras** usam o texto oficial da landing da campanha — pode publicar como está.

As **duas últimas** têm borda laranja tracejada e a tarja **PROPOSTA EM ESTUDO**.
São sugestões minhas, ainda não aprovadas. Duas saídas:

- **Aprovar:** apague o `<span class="bg__marca">Proposta em estudo</span>` e remova
  `bg__card--proposta` da lista de classes do `<article>`.
- **Descartar:** apague o `<article>` inteiro, do `<article` até o `</article>`.

**Quando resolver as duas**, apague o aviso amarelo: procure por `nota nota--revisar`
dentro da seção `#bandeiras` e apague o parágrafo inteiro.

---

## 4b. Atualizar o valor da vaquinha

O valor arrecadado **não é puxado automaticamente** da plataforma — precisa ser digitado.
Em `assets/js/dados.js`, procure o bloco `vaquinha`:
...
Troque `arrecadado` pelo total real (só números, **sem** `R$`, sem ponto e sem vírgula:
sete mil e seiscentos se escreve `7600`) e atualize `atualizadoEm` com a data da conferência.
A barra de progresso e a porcentagem se ajustam sozinhas.

> Combine com a equipe uma rotina de atualização — duas vezes por semana já basta.
> Valor desatualizado no ar passa a impressão de campanha parada.

---

## 5. Trocar telefone, Instagram ou o número

Tudo isso está no começo do `assets/js/dados.js`, no bloco `config:`:
...
`5561995623696` e por `99562-3696`, e também em `assets/js/app.js`.

### Sobre o CNPJ

O site usa **68.462.311/0001-06**, que é o CNPJ de campanha informado na landing oficial.
O material impresso traz um número diferente (20.329.974/0001-20). **Confirme com o
jurídico qual é o válido antes de publicar** — identificar o responsável é exigência do TSE.
...
gere o novo QR, salve como PNG e substitua o arquivo `assets/img/qrcode.png`,
...
Substitua os arquivos dentro de `assets/img/` mantendo os **mesmos nomes**:
